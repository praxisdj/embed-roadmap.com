  ...
  React.useEffect(() => {
    ...
    function renderRoadmap(roadmap: RoadmapData) {
      ...
      const statusConfigWithColors = Object.fromEntries(
        Object.entries(statusConfig).map(([key, config]) => [
          key,
          {
            ...config,
            color:
              styles.statusColors?.[key as keyof typeof styles.statusColors] ||
              (key === "BACKLOG"
                ? "#6b7280"
                : key === "NEXT_UP"
                  ? "#3b82f6"
      ...
      Object.entries(statusConfigWithColors).forEach(([status, config]) => {
        const count = roadmap.features.filter(
          (f: Feature) => f.status === status,
        ).length;
        html += '<div class="roadmap-embed-status-card">';
        html +=
          '<div class="roadmap-embed-status-icon">' + config.icon + "</div>";
        html += '<div class="roadmap-embed-status-info">';
        html +=
          '<span class="roadmap-embed-status-label">' +
          config.label +
          "</span>";
      ...
      Object.entries(statusConfigWithColors).forEach(([status, config]) => {
        const statusFeatures = roadmap.features.filter(
          (f: Feature) => f.status === status,
        );
        ...
        html +=
          '<span class="roadmap-embed-column-icon">' + config.icon + "</span>";
        html +=
          '<span class="roadmap-embed-column-label">' +
          config.label +
          "</span>";
        html += "</div>";
        html +=
          '<div class="roadmap-embed-column-count">' +
        ...
        if (statusFeatures.length > 0) {
          statusFeatures.forEach((feature: Feature) => {
            ...
            html +=
              '<h3 class="roadmap-embed-feature-title">' +
              feature.title +
              "</h3>";
            if (feature.description) {
              html +=
                '<p class="roadmap-embed-feature-description">' +
                feature.description +
                "</p>";
            }
            html += "</div>";
            ...
            html +=
              '<span class="roadmap-embed-feature-date">' +
              new Date(feature.createdAt).toLocaleDateString() +
              "</span>";
            html +=
              '<span class="roadmap-embed-vote-badge">' +
              (feature.votes && feature.votes.length > 0
                ? feature.votes.length
                : 0) +
              " vote" +
              (feature.votes && feature.votes.length !== 1 ? "s" : "") +
              "</span>";
            html += "</div>";
            html += "</div>";
          });
          ...
          html +=
            '<div class="roadmap-embed-empty-icon">' + config.icon + "</div>";
      ...
      if (roadmap.features.length === 0) {
        ...
        html +=
          '<p class="roadmap-embed-empty-description">This roadmap is empty. Check back later for updates.</p>';
        html += "</div>";
      }
    ...
    function showError(message: string) {
      ...
      if (!root) return;
      root.innerHTML =
        '<div class="roadmap-embed-error"><p>' + message + "</p></div>";
    }
