...
import { ArrowLeft, Plus, Users, Calendar, Eye, EyeOff, Share2, Trash2 } from "lucide-react"
...
function DroppableColumn({
  ...
  return (
    <div className="flex flex-col space-y-4">
      {/* Column Header */}
      <div className={`flex items-center justify-between p-3 rounded-lg border transition-all duration-300 ease-in-out ${
        shouldHighlight
          ...
          {features.length === 0 && (
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-center text-muted-foreground">
                ...
                <p className={`text-xs mt-1 transition-colors duration-300 ease-in-out ${
                  shouldHighlight ? 'text-blue-500/70 dark:text-blue-400/70' : 'opacity-60'
                }`}>
                  {shouldHighlight ? 'Drop here to move feature' : 'Drop features here'}
                </p>
              </div>
            </div>
          )}
...
function DeleteDropZone({ isDragging, isOverDeleteZone }: { isDragging: boolean; isOverDeleteZone: boolean }) {
  const { setNodeRef, isOver } = useDroppable({
    id: 'delete-zone',
  })
...
export default function RoadmapPage() {
  ...
  const handleAddFeatureToStatus = (status: string) => {
    setNewFeature({ title: "", description: "", status: status as keyof typeof Status })
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    console.log('Drag end - Feature ID:', featureId, 'Drop zone ID:', dropZoneId)
    ...
    if (!Object.keys(Status).includes(newStatus)) {
      ...
      if (over.data.current && over.data.current.sortable && over.data.current.sortable.containerId) {
        newStatus = over.data.current.sortable.containerId
        console.log('Using fallback status from containerId:', newStatus)
      } else {
        console.error('Invalid drop zone:', dropZoneId, 'Valid zones:', Object.keys(Status))
        return
      }
    }
    ...
    console.log('Updating feature status from', feature.status, 'to', newStatus)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-bold text-foreground">{roadmap.name}</h1>
              <Badge variant={roadmap.isPublic ? "default" : "secondary"} className="ml-2">
                {roadmap.isPublic ? (
                  <>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              ...
              <Dialog open={isDeletedFeaturesOpen} onOpenChange={setIsDeletedFeaturesOpen}>
                <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-bold">Deleted Features</DialogTitle>
                    <DialogDescription>
                      View, search, and restore deleted features. You can edit and undelete features from this list.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto">
                      {deletedFeatures
                        .filter(feature =>
                          feature.title.toLowerCase().includes(deletedSearchTerm.toLowerCase()) ||
                          (feature.description && feature.description.toLowerCase().includes(deletedSearchTerm.toLowerCase()))
                        )
                        .map((feature) => (
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-4 mt-2 text-xs text-muted-foreground">
                                  <span>Deleted: {new Date(feature.createdAt).toLocaleDateString()}</span>
                                  <span>Votes: {feature.votes?.length || 0}</span>
                                </div>
                              </div>
                              <div className="flex items-center space-x-2 ml-4">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setEditFeatureData({
                                      title: feature.title,
                                      description: feature.description || "",
                                      status: feature.status as "BACKLOG" | "NEXT_UP" | "IN_PROGRESS" | "DONE"
                                    })
                                  }}
                                >
                                  Edit
                                ...
                                <Button
                                  variant="default"
                                  size="sm"
                                  onClick={() => handleUndeleteFeature(feature.id)}
                                >
                                  Restore
                                </Button>
                              </div>
                      ...
                      {deletedFeatures.length > 0 && deletedFeatures.filter(feature =>
                        feature.title.toLowerCase().includes(deletedSearchTerm.toLowerCase()) ||
                        (feature.description && feature.description.toLowerCase().includes(deletedSearchTerm.toLowerCase()))
                      ).length === 0 && (
                        <div className="text-center py-8 text-muted-foreground">
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  <DialogTitle className="text-2xl font-bold">Add New Feature</DialogTitle>
                  <DialogDescription>
                    Create a new feature for your roadmap. You can always edit the status and details later.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewFeature(prev => ({ ...prev, title: e.target.value }))}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewFeature(prev => ({ ...prev, description: e.target.value }))}
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-muted-foreground">
              ...
              {isUpdatingStatus && <span className="ml-2 text-blue-500">• Updating...</span>}
            </div>
          </div>
          ...
          {/* Delete Drop Zone */}
          <DeleteDropZone isDragging={isDragging} isOverDeleteZone={isOverDeleteZone} />
      {/* Edit Feature Dialog */}
      {editingFeature && (
        <Dialog open={!!editingFeature} onOpenChange={closeEditDialog}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">Edit Feature</DialogTitle>
              <DialogDescription>
                Update the details of your feature. Changes will be saved immediately.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditFeatureData(prev => ({ ...prev, title: e.target.value }))}
                  className="border-border focus:ring-2 focus:ring-primary/20"
                />
              </div>
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditFeatureData(prev => ({ ...prev, description: e.target.value }))}
                  className="border-border focus:ring-2 focus:ring-primary/20 min-h-[100px]"
                />
              </div>
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(value: keyof typeof Status) =>
                    setEditFeatureData(prev => ({ ...prev, status: value }))
                  }
                >
                  <SelectTrigger className="border-border focus:ring-2 focus:ring-primary/20">
                    <SelectValue placeholder="Select status" />
        ...
        {features.length === 0 && (
          <Card className="text-center py-16">
            <CardContent>
              ...
              <h3 className="text-xl font-semibold text-foreground mb-2">No features yet</h3>
              <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                Start building your roadmap by adding the first feature. Break down your project into manageable pieces.
              </p>
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="bg-primary hover:bg-primary/90">
                ...
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    <DialogTitle className="text-2xl font-bold">Add Your First Feature</DialogTitle>
                    <DialogDescription>
                      Create your first feature to start building your roadmap. You can always edit the status and details later.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2">
